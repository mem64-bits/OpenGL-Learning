...
IGNORE_LIST = {
    "build", "out", ".git", ".vs", ".vscode", "external",
    "BuildScripts", "cmake-build-debug", "__pycache__", "venv"
}
...
def run(target_dir):
    ...
    for item in all_items:
        ...
        if os.path.isdir(full_path) and item not in IGNORE_LIST and not item.startswith("."):
            valid_subdirs.append(item)

    valid_subdirs.sort()

    if not valid_subdirs:
        print("⚠️  No valid subdirectories found.")
        return
